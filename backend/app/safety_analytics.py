...
class SafetyAnalyticsService:
    ...
    def __init__(self, repository: Any, environment_context: Any, obstacle_service: Any) -> None:
    ...
    @classmethod
    def _inside_geometry(cls, position: dict[str, float], geometry: dict[str, Any]) -> bool:
        ...
        if kind == "SPHERE":
            return math.sqrt((x-cx)**2 + (y-cy)**2 + (z-cz)**2) <= float(geometry.get("radius_m", 0.0))
        if kind == "BOX":
            return (
    ...
    @staticmethod
    def _classification(clearance: float, ttc: float | None, safety_radius: float) -> tuple[str, str]:
    ...
    def _upsert_violation(
        self, telemetry: dict[str, Any], constraint: dict[str, Any], violation_type: str,
        severity: str, measured: float | None, limit: float | None, unit: str | None,
        position: dict[str, float], details: dict[str, Any],
    ) -> dict[str, Any]:
        ...
        exceedance = abs(measured - limit) if measured is not None and limit is not None else None
        now = self._now()
        with self._lock, self._connect() as connection:
            ...
            if row:
                start = datetime.fromisoformat(str(row["start_timestamp_utc"]).replace("Z", "+00:00"))
                current = datetime.fromisoformat(timestamp.replace("Z", "+00:00"))
                duration = max(0.0, (current - start).total_seconds())
                maximum = max(float(row["maximum_exceedance"] or 0.0), float(exceedance or 0.0))
                connection.execute(
                    ...
                    WHERE violation_id=?""",
                    (timestamp, duration, measured, limit, maximum, self._json(position),
                     self._json(details), now, row["violation_id"]),
                )
                ...
                connection.execute(
                    ...
                    (violation_id, active_key, mission_id, vehicle_id, constraint_id, violation_type,
                     severity, "OPEN", timestamp, timestamp, 0.0, 1, measured, limit, exceedance,
                     unit, self._json(position), self._json(details), now, now),
                )
            result = connection.execute(
                "SELECT * FROM constraint_violations WHERE violation_id=?", (violation_id,)
            ).fetchone()
        return self._decode_violation(result)

    def _resolve_inactive(self, mission_id: str, vehicle_id: str, active_keys: set[str], timestamp: str) -> list[dict[str, Any]]:
        resolved: list[dict[str, Any]] = []
        with self._lock, self._connect() as connection:
            rows = connection.execute(
                """SELECT * FROM constraint_violations
                WHERE mission_id=? AND vehicle_id=? AND active_key IS NOT NULL
                AND status IN ('OPEN','ONGOING')""", (mission_id, vehicle_id)
            ).fetchall()
            for row in rows:
                ...
                connection.execute(
                    ...
                    (timestamp, max(0.0, (end-start).total_seconds()), self._now(), row["violation_id"]),
                )
                updated = connection.execute(
                    "SELECT * FROM constraint_violations WHERE violation_id=?", (row["violation_id"],)
                ).fetchone()
                resolved.append(self._decode_violation(updated))
        return resolved

    def analyse(self, telemetry: dict[str, Any], interaction: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        for constraint in snapshot.get("constraints") or []:
            ...
            if ctype == "SPEED_LIMIT" and inside and limit is not None:
                measured, violated, violation_type = speed, speed > limit, "SPEED_LIMIT_VIOLATION"
            elif ctype == "NO_ENTRY_ZONE":
                measured, violated, violation_type = 1.0 if inside else 0.0, inside, "NO_ENTRY_ZONE_VIOLATION"
                limit = 0.0
            elif ctype == "MAXIMUM_ALTITUDE" and limit is not None:
                measured, violated, violation_type = position["z_m"], position["z_m"] > limit, "ALTITUDE_LIMIT_VIOLATION"
                ...
                measured, violated, violation_type = 1.0 if inside else 0.0, not inside, "MISSION_BOUNDARY_EXIT"
                limit = 1.0
            elif ctype == "REQUIRED_CORRIDOR":
                measured, violated, violation_type = 1.0 if inside else 0.0, not inside, "REQUIRED_CORRIDOR_EXIT"
                limit = 1.0
            elif ctype == "BATTERY_RETURN_THRESHOLD" and limit is not None:
                measured, violated, violation_type = battery, battery < limit, "LOW_BATTERY_RETURN_VIOLATION"

            if not violated or violation_type is None:
                continue
            severity = "CRITICAL" if ctype in {"NO_ENTRY_ZONE", "MISSION_BOUNDARY"} else "WARNING"
            key = f"{mission_id}|{vehicle_id}|{constraint.get('constraint_id', violation_type)}|{violation_type}"
            active_keys.add(key)
            violations.append(self._upsert_violation(
                telemetry, constraint, violation_type, severity, measured, limit,
                constraint.get("unit"), position,
                {"constraint_name": constraint.get("name"), "constraint_type": ctype},
            ))

        resolved = self._resolve_inactive(mission_id, vehicle_id, active_keys, str(telemetry["timestamp_utc"]))
        near_miss = self._record_near_miss(telemetry, interaction, position) if interaction else None
        return {"violations": violations, "resolved": resolved, "near_miss": near_miss}

    def _record_near_miss(self, telemetry: dict[str, Any], interaction: dict[str, Any], position: dict[str, float]) -> dict[str, Any] | None:
        ...
        with self._lock, self._connect() as connection:
            connection.execute(
                ...
                (record["near_miss_id"], record["mission_id"], record["vehicle_id"],
                 record["obstacle_id"], record["interaction_id"], record["timestamp_utc"],
                 record["classification"], record["severity"], record["clearance_m"],
                 record["time_to_collision_s"], record["closing_speed_mps"],
                 self._json(position), self._json(record["details"])),
            )
            row = connection.execute(
    ...
    def list_violations(self, mission_id: str, violation_type: str | None = None,
                        status: str | None = None, limit: int = 5000) -> list[dict[str, Any]]:
        ...
        sql += " ORDER BY start_timestamp_utc DESC LIMIT ?"; params.append(limit)
        with self._connect() as connection:
            return [self._decode_violation(row) for row in connection.execute(sql, params).fetchall()]

    def list_near_misses(self, mission_id: str, classification: str | None = None,
                         limit: int = 5000) -> list[dict[str, Any]]:
        sql = "SELECT * FROM near_miss_events WHERE mission_id=?"
        params: list[Any] = [mission_id]
        if classification:
            sql += " AND classification=?"; params.append(classification)
        sql += " ORDER BY timestamp_utc DESC LIMIT ?"; params.append(limit)
        with self._connect() as connection:
            return [self._decode_near_miss(row) for row in connection.execute(sql, params).fetchall()]
    ...
    def constraint_summary(self, mission_id: str) -> dict[str, Any]:
        ...
        return {"mission_id": mission_id, "total": sum(int(r["count"]) for r in rows),
    ...
    def safety_summary(self, mission_id: str) -> dict[str, Any]:
        with self._connect() as connection:
            near = connection.execute(
                ...
                FROM near_miss_events WHERE mission_id=?""", (mission_id,)
            ).fetchone()
    ...
    def vehicle_constraint_status(self, vehicle_id: str) -> dict[str, Any]:
        ...
        return {"vehicle_id": vehicle_id, "active_violations": [self._decode_violation(r) for r in rows]}
