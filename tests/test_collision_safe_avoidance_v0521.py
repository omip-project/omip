...
def clearance(state, scenario):
    ...
    return (
        ((state["x_m"] - ox) ** 2 + (state["y_m"] - oy) ** 2) ** 0.5 - radius - safety
    )


def test_large_obstacle_never_returns_inside_safety_envelope():
    scenario = json.loads(
        (ROOT / "scenarios" / "ugv_large_obstacle_safe_stop.json").read_text()
    )
...
def test_two_obstacles_selects_a_safe_direction():
    scenario = json.loads(
        (ROOT / "scenarios" / "ugv_two_obstacles_direction_choice.json").read_text()
    )
    active = [
        motion_state(t, scenario, "GROUND_VEHICLE", PROFILE["parameters"])
        for t in [8, 9, 10, 11, 12]
    ]
    assert any(item["avoidance_active"] for item in active)
    assert any(
        item["avoidance_direction"] in {"LEFT", "RIGHT", "STOP"} for item in active
    )
