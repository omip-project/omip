...
def test_storage_summary_pagination_export_backup_and_delete(tmp_path: Path) -> None:
    ...
    with TestClient(main.app) as client:
        assert (
            client.post(
                "/api/v1/vehicles",
                json={"vehicle_id": "STORE-VEH", "vehicle_name": "Storage vehicle"},
            ).status_code
            == 201
        )
        assert (
            client.post(
                "/api/v1/vehicles/STORE-VEH/sensors",
                json={
                    "sensor_id": "STORE-GNSS",
                    "sensor_name": "GNSS",
                    "sensor_type": "GNSS",
        ...
        page = client.get(
            "/api/v1/missions/STORE-MISSION/telemetry/page?page=1&page_size=1"
        )
        ...
        raw_page = client.get(
            "/api/v1/missions/STORE-MISSION/raw-messages/page?page=2&page_size=1"
        )
        ...
        policy = client.put(
            "/api/v1/storage/retention-policy", json={"raw_messages_days": 45}
        )
        ...
        job = client.post(
            "/api/v1/export-jobs",
            json={"mission_id": "STORE-MISSION", "export_format": "package"},
        )
        ...
        assert (
            client.get(
                f"/api/v1/storage/backups/{backup.json()['backup_id']}/download"
            ).status_code
            == 200
        )
        ...
        assert (
            client.delete("/api/v1/missions/STORE-MISSION?confirm=wrong").status_code
            == 409
        )
